...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def _apply_tenant_scope(self, query: Any, tenant_id: Optional[uuid.UUID], bypass_tenant_scope: bool = False) -> Any:
        # Check if the model uses TenantMixin
        if issubclass(self.model, TenantMixin):
            if not bypass_tenant_scope:
    ...
    async def get(
        ...
        query = self._apply_tenant_scope(query, tenant_id, bypass_tenant_scope)
    ...
    async def delete(
        ...
        # We first get the item to ensure it exists and matches scope
        item = await self.get(id, tenant_id=tenant_id, bypass_tenant_scope=bypass_tenant_scope)
        if not item:
            return False
